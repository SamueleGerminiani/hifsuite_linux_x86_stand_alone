...
// returns true if the outputs are the same
bool checkOutput(const test::test_iostruct& golden,
                 const test::test_iostruct& faulty) {
  if (golden.c_1 != faulty.c_1 || golden.c_2 != faulty.c_2 ||
      golden.c_3 != faulty.c_3) {
    return 0;
  }
  return 1;
}
...
bool areEquivalent(const Trace& t1, const Trace& t2) {
  ...
  for (size_t i = 0; i < t1.size(); ++i) {
    if (t1[i].a_1 != t2[i].a_1 || t1[i].a_2 != t2[i].a_2 ||
        t1[i].a_3 != t2[i].a_3 || t1[i].b_1 != t2[i].b_1 ||
        t1[i].b_2 != t2[i].b_2 || t1[i].b_3 != t2[i].b_3 ||
        t1[i].c_1 != t2[i].c_1 || t1[i].c_2 != t2[i].c_2 ||
        t1[i].c_3 != t2[i].c_3) {
      return false;
    }
  }
...
int main() {
  ...
  // spec0
  instanceToNumberOfFaults.push_back(
      test_instance.spec_sbm0.hif_fault_node.number);
  // spec1
  instanceToNumberOfFaults.push_back(
      test_instance.spec_sbm1.hif_fault_node.number);
  ...
  for (auto [instance, fault] : uncoveredFaults) {
    printf("Fault n %ld of instance %ld not observed\n", instance, fault);
  }
